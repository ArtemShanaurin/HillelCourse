...
public class WineCocktailsPage extends AbstractPage{
    ...
    public int countElementsRedWine(){
        List<WebElement> redWineList = DriverProvider.INSTANCE.getDriver().findElements(By.cssSelector(".tile.recipe-tile[data-types=\"red\"]"));
    ...
    public void scrollToRaspberryRose(){
        ...
        ((JavascriptExecutor) DriverProvider.INSTANCE.getDriver()).executeScript("arguments[0].scrollIntoView(true);", scrollToRaspberryRoseReceipt);
        scrollToRaspberryRoseReceipt.click();
    }

    public String getCurrentUrl(){
        String currentUrl = DriverProvider.INSTANCE.getDriver().getCurrentUrl();
        return currentUrl;
    }

    public int countElementsRedAndSparklingWine(){
        List<WebElement> wineList = DriverProvider.INSTANCE.getDriver().findElements(By.cssSelector("[class=\"tile recipe-tile\"]"));
        int count = 0;
